...
class KeyParser:
    ...
    def __init__(self,evt):
        ...
        self.keyname = keyMap.get(self.keycode, None)
        if self.keyname is None:
            if "unicode" in wx.PlatformInfo:
                self.keycode = evt.GetUnicodeKey()
                if self.keycode <= 127:
                    self.keycode = evt.GetKeyCode()
                self.keyname = unichr(evt.GetUnicodeKey())
                    
            elif self.keycode < 256:
                if self.keycode == 0:
                    self.keyname = "NUL"
                elif self.keycode < 27:
                    self.keyname = "Ctrl-%s" % chr(ord('A') + self.keycode-1)
                else:
                    self.keyname = "%s" % chr(self.keycode)
            else:
                self.keyname = "unknown (%s)" % self.keycode
